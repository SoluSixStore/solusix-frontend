...
export const MagicTextReveal: React.FC<MagicTextRevealProps> = ({
  ...
  }, [isHovered, showText, spread, speed, globalDpr, updateParticles, renderParticles]);  // Handle resize
  useEffect(() => {
    const handleResize = () => {
      if (wrapperRef.current && textDimensions.width && textDimensions.height) {
        // For inline text, use minimal padding
        const basePadding = Math.max(fontSize * 0.1, 4);
        
        const minWidth = Math.max(textDimensions.width + basePadding * 2, textDimensions.width);
        const minHeight = Math.max(textDimensions.height + basePadding * 2, textDimensions.height);
