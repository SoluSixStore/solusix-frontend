...
type SpanProps = React.ComponentProps<typeof motion.span> & {
  speed?: number;
  onAnimationComplete?: () => void;
};
...
function AppleHelloPortugueseEffect({
  className,
  speed = 1,
  onAnimationComplete,
  ...props
  ...
  return (
    <motion.span
      className={cn("inline-block text-lime font-semibold", className)}
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      transition={{ duration: calc(0.5) }}
      onAnimationComplete={onAnimationComplete}
      {...props}
    >
      Olá
